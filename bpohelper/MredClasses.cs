...
namespace bpohelper
{
    ...
    public class MLSListing
    {
        ...
        public string UniversalLandUse()
        {
            //realist name for whether property is Single Family REsidential or Condo/Common
            string uluCode = "";

            if (this.GetType().ToString().Contains("Detached"))
            {
                uluCode = "Detached";
            }

            if (this.GetType().ToString().Contains("Attached"))
            {
                uluCode = "Attached";
            }
        ...
        public string AdditionalSalesInfo()
        {
            ...
            if (String.IsNullOrWhiteSpace(mlsHtmlFields["additionalSalesInfo"].value))
            {
                mlsHtmlFields["additionalSalesInfo"].value = Regex.Match(rawData, @"Additional&nbsp;Sales&nbspInformation:<.td><td class=.value.>(\d).\d").Groups[1].Value;
            }
        ...
        internal void SetMlsFields()
        {
            ...
            if (String.IsNullOrWhiteSpace(mlsHtmlFields["numSpaces"].value))
            {
                try
                {
        ...
        public string TransactionType
        {
            get
            {
                ...
                {
                    //active listing
                    if (this.AdditionalSalesInfo().Contains("REO"))
                    {
                        transactionType = "REO";
                    }
                    if (this.AdditionalSalesInfo().Contains("Short Sale"))
                    {
        ...
        public string DOM
        {
            ...
            get
            {
                if (String.IsNullOrWhiteSpace(mlsHtmlFields["daysOnMarket"].value))
                {
                    return Regex.Match(rawData, @"List.&nbsp;Mkt&nbsp;Time:<.td><td class=.value.>(\d+)").Groups[1].Value;
                }
                return mlsHtmlFields["daysOnMarket"].value;
            }
        ...
        public string FullBathCount
        {
            get
            {
                ...
                try
                {
                    return mlsHtmlFields["bathrooms"].value[0].ToString();
                }
                catch
                {
                    return Regex.Match(rawData, @"Total&nbsp;Baths .Full.Half.:<.td><td class=.value.>(\d).\d").Groups[1].Value;
                }
            }
        }
        public string HalfBathCount
        {
            get
            {
                ...
                {
                    theCount = Regex.Match(rawData, @"Total&nbsp;Baths .Full.Half.:<.td><td class=.value.>\d.(\d)").Groups[1].Value;
                }
        ...
        public string BathroomCount
        {
            get
            {
                ...
                try
                {
                    return mlsHtmlFields["bathrooms"].value.Replace(" ", "").Replace(@"/", ".");
                }
                catch
                {
                    return FullBathCount + "." + HalfBathCount;
                }
        ...
        public string BedroomCount
        {
            get
            {
                ...
                {
                    return Regex.Match(rawData, @"Total&nbsp;Bedrooms:<.td><td class=.value.>\d.(\d)").Groups[1].Value;
        ...
        public string NumberOfFireplaces
        {
            get
            {
                ...
                if (String.IsNullOrWhiteSpace(theCount))
                    theCount = "50";
        ...
        public int Age
        {
            get
            {
                ...
                if (s == "UNK")
                {
                    s = GlobalVar.theSubjectProperty.MainForm.SubjectYearBuilt;
                }
    ...
    public class TwoToFourUnitsListing : MLSListing
    {
        public TwoToFourUnitsListing()
        {

        }
        //static private TypeDetachedList tl = new TypeDetachedList();
        public TwoToFourUnitsListing(string s)
            : base(s)
        {
            mlsPropType = "Two to Four Units";
            SetMlsFields();
        }
        ...
        {
            base.SetAboveGradeLevels();
            //Type-Multi Unit:2 Flat
            switch (Regex.Match(rawData, @"Type-Multi&nbsp;Unit:<.td><td class=.value.>.(*?)</td>").Groups[1].Value)
            {
                case "1 Story":
                case "Raised Ranch":
                case "Coach House":
